...
class TouchTexture {
  size = 64; width = 64; height = 64; maxAge = 180; radius = 0.22; speed = 1/48;
  ...
  addTouch(point: any) {
    ...
    if (this.last) {
      ...
      vx = dx/d; vy = dy/d;
      force = Math.min((dx*dx + dy*dy) * 30000, 3.0);
    }
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Sample touch texture for interaction
          vec4 touchTex = texture2D(uTouchTexture, uv);
          float touchIntensity = touchTex.b;
          vec2 touchDir = (touchTex.rg * 2.0 - 1.0);
          
          // 1. Refractive Displacement (Glass-like)
          // Pushes the background colors away from the cursor with a lens-like distortion
          float refractionScale = 0.45;
          vec2 displacedUv = uv - touchDir * touchIntensity * refractionScale;
          
          // 2. Organic Liquid Flow
          // Adds a subtle, constant viscous movement to the UVs
          float flowTime = uTime * 0.4;
          displacedUv.x += sin(displacedUv.y * 4.0 + flowTime) * 0.015;
          displacedUv.y += cos(displacedUv.x * 4.0 + flowTime) * 0.015;
          
          // 3. Ripple Effect (Enhanced by interaction)
          vec2 center = vec2(0.5);
          float dist = length(displacedUv - center);
          float ripple = sin(dist * 15.0 - uTime * 1.2) * 0.02 * (1.0 + touchIntensity * 2.0);
          displacedUv += ripple;
          
          // Get the base gradient color
          vec3 color = getGradientColor(displacedUv, uTime);
          
          // 4. Glassmorphism Specular Highlights
          // Calculate pseudo-normals from the touch intensity to create "glossy" reflections
          float eps = 0.02;
          float h = touchIntensity;
          float hx = texture2D(uTouchTexture, uv + vec2(eps, 0.0)).b - h;
          float hy = texture2D(uTouchTexture, uv + vec2(0.0, eps)).b - h;
          
          // Create a normal vector for the "surface" of the liquid
          vec3 normal = normalize(vec3(-hx, -hy, 0.2));
          vec3 lightDir = normalize(vec3(0.5, 0.5, 1.0));
          float specular = pow(max(dot(normal, lightDir), 0.0), 24.0);
          
          // Apply highlights: brighter where the liquid is "bending" near the cursor
          color += specular * 0.35 * touchIntensity;
          
          // 5. Depth and Vibrancy (Glassy finish)
          // Increase contrast and saturation slightly in interaction areas
          color = mix(color, color * 1.15 + 0.05, touchIntensity * 0.4);
          
          // Add film grain for texture
          color += grain(displacedUv, uTime) * uGrainIntensity;
